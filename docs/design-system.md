...
- bare feed hero icon (no container chrome) and primary reader action emphasis
- empty/error presentation

Item title / meta / excerpt must reuse the shared `.ui-item__*` classes in **meta → linked title → optional excerpt** order (same as the result preview). Section rhythm uses shared `layout-section-divided` on the item list and bottom metadata utility — do not add feed-only section separators.

### Feed item legal-minimal surface

The feed item list is index-only — not a republication surface:

- linked title (`ui-item__title > a`) is the primary outbound affordance; no separate action row
- excerpt only when the item has a real title and a non-duplicative description
- title-less items use a short description label (72 chars max) with no excerpt
- excerpt cap: 96 chars when shown
- no images, bylines, tags, or inline HTML reproduction in the XSL view
...
- hero IA: `ui-eyebrow` context → `ui-headline-row` (title left, bare RSS badge right) → description → stamp (`ui-eyebrow` + `time`) → `ui-actions layout-rail-reading` (buttons only)
- feed metadata (`Source`, `Generated by`) is a bottom utility block after the item list; item count lives in the section eyebrow; omit the block when both are absent
