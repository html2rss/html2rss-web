...
Item title / meta / excerpt / actions must reuse the shared `.ui-item__*` classes in **meta → title → excerpt → actions** order (same as the result preview). Section rhythm uses shared `layout-section-divided` on the item list and bottom metadata utility — do not add feed-only section separators.
...
- feed metadata (`Items`, `Source`, `Generated by`) is a bottom utility block after the item list, not hero chrome
