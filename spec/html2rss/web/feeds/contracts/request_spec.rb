...
RSpec.describe Html2rss::Web::Feeds::Contracts::Request do
  ...
  it 'builds a static request with normalized feed name', :aggregate_failures do
    parsed = described_class.from_rack_request(request, target_kind: :static, identifier: 'legacy.json')
  ...
  it 'builds a token request with normalized token', :aggregate_failures do
    parsed = described_class.from_rack_request(request, target_kind: :token, identifier: 'token.json')
