...
RSpec.describe Html2rss::Web::Feeds::RssRenderer do
  ...
  it 'passes the normalized site title into empty-feed rendering' do
    allow(Html2rss::Web::XmlBuilder).to receive(:build_empty_feed_warning).and_return('<rss/>')

    render_empty_feed

    expect(Html2rss::Web::XmlBuilder).to have_received(:build_empty_feed_warning).with(expected_builder_args)
  end
